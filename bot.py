...
Сообщение (имя в мессенджере: {sender_name}):
---
{message_text}
---

Правила:
1. Если это НЕ заказ (вопрос, приветствие, обсуждение) — верни {{"is_order": false}}
2. Имя: берёт из сообщения, если не указано — используй {sender_name}
3. Телефон и город: если не указаны — пустая строка
4. Товары: сопоставь с списком выше, используй ТОЧНЫЕ названия из списка
   (допускай опечатки и сокращения: "форель" → "Форель", "икра гор" → "Икра горбуши")
5. Количество: если клиент написал просто название без числа — считай 1
